...
export class SupabaseStorageService implements StorageService {
	...
	async uploadFile(
		...
		const { data, error } = await supabase_full_access.storage
			.from(bucketName)
	...
	async checkFileExistence(
		...
		if (expected) {
			expect(
				fileExists,
				`Expected file '${fileName}' to exist in folder '${folderPath}', but it was not found. Available files: ${files}`
			).toBe(true);
		} else {
			expect(
				fileExists,
				`Expected file '${fileName}' to NOT exist in folder '${folderPath}', but it was found. Available files: ${files}`
			).toBe(false);
		}
	}
}
