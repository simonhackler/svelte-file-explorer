...
export class LocalStorageService implements StorageService {
	...
	async checkFileExistence(
		...
		if (expected) {
			expect(
				fileExists,
				`File '${constructedFilePath}' was expected to exist in localStorage but was not found. Key: ${storageKey}`
			).toBe(true);
		} else {
			expect(
				fileExists,
				`File '${constructedFilePath}' was expected not to exist in localStorage but was found. Key: ${storageKey}`
			).toBe(false);
		}
	}
}
